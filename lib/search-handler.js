...
const { MemoryCache, CACHE_TTL_MS } = require("./cache");
const { researchWeb, academicWindow, SCHEMA_VERSION } = require("./research");
...
function cacheKey(input, model, now) {
  return createHash("sha256").update(JSON.stringify([
    SCHEMA_VERSION, model, academicWindow(new Date(now)).admissionYear,
    ...["schoolType", "university", "major", "admissionTrack"].map((key) => input[key].toLocaleLowerCase("ko"))
  ])).digest("hex");
}

function createSearchHandler({ research = researchWeb, now = Date.now, cache = new MemoryCache({ now }) } = {}) {
  ...
  return async function handler(req, res) {
    ...
    try {
      ...
      const key = cacheKey(input, model, now());
      ...
      if (!entry) {
        ...
        if (!pending.has(key)) {
          limit(req);
          const promise = (async () => {
            const value = await research(input, { model, now: new Date(now()) });
