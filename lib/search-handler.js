...
const { resolveSearch } = require("./resolve-search");
...
function validateInput(body) {
  ...
  if (Object.keys(body).some((key) => ![...keys, "aliases"].includes(key))) throw httpError(400, "지원하지 않는 검색 항목이 있습니다.");
  const input = {};
  ...
  if (body.aliases !== undefined) {
    if (!body.aliases || typeof body.aliases !== "object" || Array.isArray(body.aliases) ||
        Object.keys(body.aliases).some((key) => !["university", "major", "admissionTrack"].includes(key)))
      throw httpError(400, "이전 명칭 목록이 올바르지 않습니다.");
    input.aliases = {};
    for (const key of ["university", "major", "admissionTrack"]) {
      const values = body.aliases[key] || [];
      if (!Array.isArray(values) || values.length > 8 || values.some((value) => typeof value !== "string" || !value.trim() || value.length > 120 || /[\u0000-\u001f\u007f<>]/.test(value)))
        throw httpError(400, "이전 명칭 목록이 올바르지 않습니다.");
      input.aliases[key] = values.map((value) => value.normalize("NFKC").replace(/\s+/g, " ").trim());
    }
  }
  return input;
}
...
function createSearchHandler(stage, { research = stage === "resolve" ? resolveSearch : researchStage, now = Date.now, cache = createCache({ namespace: stage, now }) } = {}) {
  if (!["resolve", "overview", "questions", "reviews"].includes(stage)) throw new Error("UNKNOWN_SEARCH_STAGE");
