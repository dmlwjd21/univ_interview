...
function verifiedQuestions(items, sources, year) {
  return (Array.isArray(items) ? items : []).flatMap((item) => {
    ...
    if (!source || source.sourceType !== item.sourceType || !text(item.question) || (year && source.year !== null && source.year !== year)) return [];
...
function normalizeStage(stage, raw, evidence, input, window, now, knownDomain) {
  ...
  if (stage === 'overview') {
    ...
    return { ...base, officialDomain: domain, officialOffice: officeUrl && evidence.has(officeUrl) ? { title: text(raw.officialOffice.title, 180), url: officeUrl, domain } : null, guide, interviewOverview, latestQuestions, admissionResults };
  }
  if (stage === 'questions') {
...
function instructions(stage, input, window, now, domain) {
  ...
  if (stage === 'questions') return `${common} ${window.years.join(', ')}학년도 실제 면접 및 구술고사 문항을 연도별 검색. ${input.university} ${input.major} ${input.admissionTrack} '면접 및 구술고사 문항' '선행학습 영향평가'. ${domain ? `site:${domain} 위주로 검색.` : '공식 입학처를 먼저 찾는다.'} 대학 공식 기출(A) 우선, 교육청 수합(B) 보완. 해당 학과/전형의 실제 질문만 연도당 최대 3개 기록하고 못 찾은 연도는 생략. PDF 본문을 확인하지 못한 질문은 만들지 않는다.`;
