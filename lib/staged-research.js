...
function instructions(stage, input, window, now, domain) {
  ...
  if (stage === 'questions') return `${common} ${window.years.join(', ')}학년도 실제 면접 및 구술고사 문항을 연도별 검색. 먼저 대학 전체 공식 '면접 및 구술고사 문항' PDF/게시물, '선행학습 영향평가', '면접 예시문항', '전년도 면접문항'을 찾고 해당 모집단위가 적용받는 문항 유형을 자료 안에서 확인한다. 현재 명칭 ${input.university} ${input.major} ${input.admissionTrack} 외에 공식 자료에서 확인된 과거 명칭 후보 ${JSON.stringify(input.aliases || {})}도 검색하되, 이름만 비슷한 다른 전형을 섞지 말고 평가방식의 연속성을 공식 자료에서 검증한다. ${domain ? `site:${domain} 위주로 검색.` : '공식 입학처를 먼저 찾는다.'} 대학 공식 기출(A) 우선, 교육청 수합(B) 보완. 그 뒤 공개 후기(C)는 참고만 한다. 해당 학과/전형의 실제 질문만 연도당 최대 3개 기록하고 못 찾은 연도는 생략. PDF 본문을 확인하지 못한 질문은 만들지 않는다.`;
